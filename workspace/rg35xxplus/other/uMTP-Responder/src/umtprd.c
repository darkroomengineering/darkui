...
#include <sys/prctl.h>
#include <sys/file.h>
#include <fcntl.h> /* darkUI patch: open() for the mqueue-less lock fallback */
...
int main(int argc, char *argv[])
{
	...
	if (fd < 0)
	{
		/* darkUI patch: the Anbernic H700 4.9 BSP kernel is built without
		   CONFIG_POSIX_MQUEUE, so mq_open() always fails with ENOSYS. The
		   queue fd is only needed here as a flock() target for single-
		   instance detection (the -cmd IPC channel degrades gracefully
		   elsewhere), so fall back to a plain lock file instead of dying. */
		fd = open("/tmp/umtprd.lock", O_RDWR | O_CREAT, 0600);
		if (fd < 0)
		{
			exit(1);
		}
