...
//
//	C scalers
//	(only the diagonal xmul==ymul variants are reachable; off-diagonal wrappers
//	and the 32bpp family were unused dead code and have been removed. The Nx_c16
//	family functions below remain as static helpers because the NEON scalers use
//	them as an unaligned-address fallback.)
//
static void scale1x_c16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp, uint32_t ymul) {
...
static void scale2x_c16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp, uint32_t ymul) {
	if (!sw||!sh||!ymul) return;
...
static void scale2x2_c16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp) {
	scale2x_c16(src, dst, sw, sh, sp, dw, dh, dp, 2); }

static void scale3x_c16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp, uint32_t ymul) {
	if (!sw||!sh||!ymul) return;
	uint32_t x, dx, pix, dpix1, dpix2, swl = sw*sizeof(uint16_t);
...
static void scale3x3_c16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp) {
	scale3x_c16(src, dst, sw, sh, sp, dw, dh, dp, 3); }

static void scale4x_c16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp, uint32_t ymul) {
	if (!sw||!sh||!ymul) return;
	uint32_t x, dx, pix, dpix1, dpix2, swl = sw*sizeof(uint16_t);
...
static void scale4x4_c16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp) {
	scale4x_c16(src, dst, sw, sh, sp, dw, dh, dp, 4); }

static void scale5x_c16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp, uint32_t ymul) {
	if (!sw||!sh||!ymul) return;
	uint32_t x, dx, pix, dpix1, dpix2, swl = sw*sizeof(uint16_t);
...
static void scale6x_c16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp, uint32_t ymul) {
	if (!sw||!sh||!ymul) return;
	uint32_t x, dx, pix, dpix1, dpix2, swl = sw*sizeof(uint16_t);
...
//
//	NEON memcpy (dst/src must be aligned 4, size must be aligned 2)
//
...
//
//	NEON scalers
//	(only the diagonal xmul==ymul variants are reachable externally; off-diagonal
//	wrappers, the 32bpp family, and the generic function-pointer dispatchers were
...
static void scale5x_n16line(void* src, void* dst, uint32_t swl) {
	asm volatile (
...
static void scale5x_n16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp, uint32_t ymul) {
	if (!sw||!sh||!ymul) return;
	uint32_t swl = sw * sizeof(uint16_t);
	uint32_t dwl = swl*5;
...
static void scale6x_n16line(void* src, void* dst, uint32_t swl) {
	asm volatile (
	"	bic r4, %2, #15		;"	// r4 = swl16
	"	add r3, %0, %2		;"	// r3 = lineend offset
	"	add r4, %0, r4		;"	// r4 = x16bytes offset
...
static void scale6x_n16(void* __restrict src, void* __restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dw, uint32_t dh, uint32_t dp, uint32_t ymul) {
	if (!sw||!sh||!ymul) return;
	uint32_t swl = sw * sizeof(uint16_t);
	uint32_t dwl = swl*6;
	if (!sp) { sp = swl; } if (!dp) { dp = dwl; }
